...
def BiRNN(x, weights, biases, model_name, rnn_dim, con_option, avg_option, batch_size, n_classes, node_added, rnn_n_layer, cnn_base):
    ...
    # return hc
       ...
       if con_option == 1:
          ...
          l3 = conv3#maxpool2d(conv3, k=2) # 7 * 7 * 1024 -> k = 2 -> 4 * 4 * 1024 = 16384
          #l3 = maxpool2d(l3, k=2)    # 4 * 4 * 1024 -> k = 2 -> 2 * 2 * 1024 = 4096
          if cnn_base == "vgg":
             l3 = maxpool2d(l3, k=2) ################################################################################# VGG
          # block4: 7 * 7 * 2048 = 100352
          l4 = conv4 #maxpool2d(conv4, k=2) # 7 * 7 * 2048 -> k = 2 -> 4 * 4 * 2048 = 32768
          ###if cnn_base == "res":
       ...
       if args.resi:
          ...
          return [l4,l3,l2,l1], var_names_cnn
...
if model_name == 'hc':
   ...
   pred, var_names_cnn = BiRNN(x, None, None, model_name, rnn_dim, con_option, avg_option, batch_size, n_classes, node_added, rnn_n_layer, args.base)
   ###########################  Capsule
   n_routing   = 3  
   print (len(pred))
   with tf.name_scope('HC_CAPS'):
    for i in range(len(pred)):
       # Now, let’s create the encoder part of the network:
        print (i,pred[i])
        primary_capsule = PrimaryCapsule(n_vector=8, n_channel=32, n_kernel_size=4, n_stride=1)(pred[i])
        digit_capsule = CapsuleLayer(n_capsule=n_classes+2, n_vec=16, n_routing=n_routing, name='digit_capsule'+str(i))(primary_capsule)
        output_capsule = LengthLayer(name='output_capsule'+str(i))(digit_capsule)
        pred[i] = output_capsule
   var_names_caps = [i for i in tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='HC_CAPS')]
   ...
   primary_capsule = PrimaryCapsule(n_vector=8, n_channel=32, n_kernel_size=7, n_stride=1)(pred)
   ...
   digit_capsule = CapsuleLayer(n_capsule=n_classes, n_vec=16, n_routing=n_routing, name='digit_capsule')(primary_capsule)
...
#           pred_seq_soft.append(tf.nn.sigmoid(pred_seq[i]))
          cost = margin_loss(y_encoded[:,i,:],pred_seq[i]) * tf.ones([1,1], tf.float32)*w[i]
          pred_seq_soft.append(tf.nn.sigmoid(pred_seq[i]))            
       else:
#           cost += tf.nn.sigmoid_cross_entropy_with_logits(logits=pred_seq[i], labels=y_encoded[:,i,:]) * tf.ones([1,1], tf.float32)*w[i]
#           pred_seq_soft.append(tf.nn.sigmoid(pred_seq[i]))
   ...
   if args.opt == "adam":
      ...
      optimizer_cap = tf.train.AdamOptimizer(learning_rate=learning_rate).minimize(cost,var_list=var_names_caps)
   elif args.opt == "sgd":
      optimizer = tf.train.MomentumOptimizer(learning_rate=learning_rate,momentum=0.9).minimize(cost)
  ...
  with tf.Session(config=config) as sess:
    ...
    while (epoch < n_epoch):
        ...
        def infer_val():
           ...
           if args.server == 'nu':
              infer_dir = save_dir + "code-tf/hc/tmp/hc_finetuned/" + args.idx + "-infer-ep-" + str(epoch-1) + "_val/"
           else:
              infer_dir = save_dir + "code/hc-tf/hc/tmp/hc_finetuned/" + args.idx + "-infer-ep-" + str(epoch-1) + "_val/"
            ...
            while (repeat < n_repeat):
                ...
                for step in range(0,int(len(batch_x)/batch_size)):
                    ...
                    if tr_now == "RNN":
                       ...
                       sess.run(optimizer_cap, feed_dict={x: batch_x_temp, y_encoded: batch_y_temp})
                    elif tr_now == "CNN":
  ...
  with tf.Session() as sess:
    ...
    while (epoch < n_epoch):
        ...
        def infer_val_():
           print("Start ..epoch {} ..train {} ..validation {}".format(epoch, len(te_filenames), len(te_filenames)))
           if args.server == 'nu':
              infer_dir = save_dir + "code-tf/hc/tmp/cnn_finetuned/" + args.idx + "-infer-ep-" + str(epoch-1) + "_val/"
           else:
              infer_dir = save_dir + "code/hc-tf/hc/tmp/cnn_finetuned/" + args.idx + "-infer-ep-" + str(epoch-1) + "_val/"
           if not os.path.exists(infer_dir):
              os.makedirs(infer_dir)
           print(datetime.now())
           for batch_idx in range(len(te_filenames)):
               ...
               for step in range(0,int(len(batch_x_te)/batch_size)):
                   ...
                   #batch_y_path_temp_te = batch_y_path_te[(step*batch_size):(step+1)*batch_size]
                   loss, pred_s = sess.run([cost, output_capsule], feed_dict={x: batch_x_temp_te, y: batch_y_temp_te})
                   acc, acc_p = 0, 0 
               ...
               #save_pred_resnet_cap(infer_dir,pred_soft,batch_y_te,batch_idx)
           print("Inference is done!")
           #create_checks(infer_dir)         
           #sys.exit()
        ...
        for batch_idx in range(len(train_filenames)):
            ...
            while (repeat < n_repeat):
                ...
                for step in range(0,int(len(batch_x)/batch_size)):
                    ...
                    loss, pred_s = sess.run([cost, output_capsule], feed_dict={x: batch_x_temp, y: batch_y_temp})
                    acc, acc_p = 0, 0
