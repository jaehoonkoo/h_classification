...
##          l2 = maxpool2d(l2, k=2)    # 7 * 7 * 512 -> k = 2 -> 4 * 4 * 512 = 8192
          ...
          l3 = maxpool2d(conv3, k=1) # 7 * 7 * 1024 -> k = 2 -> 4 * 4 * 1024 = 16384
          #l3 = maxpool2d(l3, k=2)    # 4 * 4 * 1024 -> k = 2 -> 2 * 2 * 1024 = 4096
          if cnn_base == "vgg":
             l3 = maxpool2d(l3, k=1) ################################################################################# VGG
          # block4: 7 * 7 * 2048 = 100352
          l4 = maxpool2d(conv4, k=1) # 7 * 7 * 2048 -> k = 2 -> 4 * 4 * 2048 = 32768
          ###if cnn_base == "res":
...
if model_name == 'hc':
   ...
   for i in range(len(pred)):
       # Now, let’s create the encoder part of the network:
        print (i,pred[i])
        primary_capsule = PrimaryCapsule(n_vector=8, n_channel=32, n_kernel_size=7, n_stride=1)(pred[i])
