...
if args.server == "nu":
   ...
   data_dir =  "/home/jkoo/data/open-images/" #"/mnt-gluster/data/jkoo/"             ## osp/wcdl0
   save_dir =  "/home/jkoo/data/" #"/mnt-gluster/data/jkoo/"                              ## osp/wcdl0
    ...
    while (epoch < n_epoch):
        ...
        def infer_test():
           ...
           if args.server == 'nu':
              ...
              infer_dir = home_dir + "code/hc-tf/hc/tmp/hc_finetuned/" + args.idx + "-infer-ep-" + str(epoch-1) + "/"
  ...
  with tf.Session() as sess:
    ...
    while (epoch < n_epoch):
        ...
        def infer_test_():
           print("Start ..epoch {} ..train {} ..validation {}".format(epoch, len(te_filenames), len(te_filenames)))
           if args.server == 'nu':
              infer_dir = home_dir + "code-tf/hc/tmp/cnn_finetuned/" + args.idx + "-infer-ep-" + str(epoch-1) + "/"
           else:
              infer_dir = home_dir + "code/hc-tf/hc/tmp/cnn_finetuned/" + args.idx + "-infer-ep-" + str(epoch-1) + "/"
           if not os.path.exists(infer_dir):
              os.makedirs(infer_dir)
           print(datetime.now())
           for batch_idx in range(len(te_filenames)):
